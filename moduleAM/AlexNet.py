...
# model.features[0] = nn.Conv2d(1, 64, kernel_size=5, stride=1, padding=2)  # 修改输入通道为1
# num_features = model.classifier[6].in_features
# model.classifier[6] = nn.Linear(num_features, 2)  # 修改输出层为二分类
#
# # 定义损失函数和优化器
...
# 将特征转换为图像格式（适配AlexNet输入：227x227单通道，AlexNet经典输入尺寸）
def to_image(tensor, size=(227, 227)):
    """将特征向量转换为指定尺寸的单通道图像（AlexNet适配227x227）"""
...
# 转换为张量并重塑为图像（AlexNet适配227x227输入）
X_train = torch.tensor(X_train, dtype=torch.float32)
X_val = torch.tensor(X_val, dtype=torch.float32)
X_test = torch.tensor(X_test, dtype=torch.float32)
X_train = to_image(X_train, size=(227, 227))  # 关键：改为AlexNet适配的227x227
X_val = to_image(X_val, size=(227, 227))
X_test = to_image(X_test, size=(227, 227))
...
def show_image(tensor):
    """可视化单张图像（去除通道维度）"""
    tensor = tensor.squeeze(0)  # (1, 227, 227) -> (227, 227)
    plt.imshow(tensor.numpy(), cmap='gray')
    plt.axis('off')
    plt.show()
...
# 创建数据加载器（AlexNet计算量小于ResNet34，可保持batch_size=32）
batch_size = 32
train_dataset = CustomDataset(X_train, y_train)
val_dataset = CustomDataset(X_val, y_val)
test_dataset = CustomDataset(X_test, y_test)
...
# -------------------------- 定义AlexNet模型（适配单通道输入和二分类）--------------------------
# 加载预训练AlexNet（兼容新版本torchvision，避免pretrained=True弃用问题）
model = models.alexnet(pretrained=True)

# 1. 适配单通道输入：修改第一个卷积层（AlexNet默认in_channels=3）
model.features[0] = nn.Conv2d(
    in_channels=1,  # 单通道输入（灰度图）
    out_channels=64,  # 保持原输出通道数（AlexNet第一层输出64）
    kernel_size=11,  # AlexNet经典卷积核大小（11x11）
    stride=4,  # 保持原步长
    padding=2,  # 保持原填充
    bias=False
)

# 2. 适配二分类：修改最后一个全连接层（AlexNet默认输出1000类）
num_features = model.classifier[6].in_features  # 获取分类头输入维度（4096）
model.classifier[6] = nn.Linear(num_features, 2)  # 替换为二分类输出

# 将模型移至GPU/CPU
model = model.to(device)

# 损失函数和优化器（AlexNet学习率可适当调整）
criterion = nn.CrossEntropyLoss()
optimizer = optim.Adam(model.parameters(), lr=0.0005)  # 学习率从0.001降至0.0005，避免过拟合
...
# 训练模型（AlexNet收敛较快，可保持30个epoch或适当减少）
...
result_df.to_csv('AlexNet_ILM_SNP.csv', index=False)
print("\n结果已保存至 AlexNet_ILM_SNP.csv")
