...
# 定义模型
model = create_model('convnext_base', pretrained=True, num_classes=2)
model.stem[0] = nn.Conv2d(1, model.stem[0].out_channels, kernel_size=5, stride=1, padding=2)  # 修改输入通道为1

# 修改输出层为二分类
num_features = model.head.fc.in_features
model.head.fc = nn.Linear(num_features, 2)  # 修改为二分类输出
