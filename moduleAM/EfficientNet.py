...
# 数据集路径
train_file = '../finalFE/ION_INDEL_Train.csv'
valid_file = '../finalFE/ION_INDEL_Valid.csv'
test_file = '../finalFE/ION_INDEL_Test.csv'
...
model.features[0][0] = nn.Conv2d(1, 32, kernel_size=5, stride=1, padding=2, bias=False)  # 修改输入通道为1
num_features = model.classifier[1].in_features
model.classifier[1] = nn.Linear(num_features, 2)  # 修改输出层为二分类
...
result_df.to_csv('NASNet_ION_INDEL.csv', index=False)
print("\n结果已保存至 NASNet_ION_INDEL.csv")
#
# # -------------------------- 关键添加：自动检测GPU并设置设备 --------------------------
...
#
#
# # 将特征转换为图像格式（适配EfficientNet-B0输入：224x224单通道）
# def to_image(tensor, size=(224, 224)):
#     feature_len = size[0] * size[1]
#     if tensor.shape[1] > feature_len:
#         tensor = tensor[:, :feature_len]
...
# X_val = torch.tensor(X_val, dtype=torch.float32)
# X_test = torch.tensor(X_test, dtype=torch.float32)
# X_train = to_image(X_train, size=(224, 224))
...
#         outputs = model(inputs)
#         # -------------------------- 关键修改：结果移回CPU（用于后续计算） --------------------------
#         scores = outputs.softmax(dim=1).cpu().numpy()[:, 1]
#         all_labels.extend(labels.cpu().numpy())
#         all_scores.extend(scores)
#
# # 计算阈值和指标
...
#     'F1_Optimal_Threshold': opt_thresh_f1,
# })
# result_df.to_csv('EfficientNetB0_ILM_INDEL.csv', index=False)
# print("\n结果已保存至 EfficientNetB0_ILM_INDEL.csv")