...
# 定义模型
model = create_model('nasnetalarge', pretrained=True, num_classes=2)

# 修改输入层以支持单通道输入（如果需要）
model.conv0 = nn.Conv2d(1, 96, kernel_size=5, stride=1, padding=2, bias=False)
...
result_df.to_csv('NASNet_ILM_SNP.csv', index=False)
print("\n结果已保存至 NASNet_ILM_SNP.csv")
