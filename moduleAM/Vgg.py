...
# model.features[0] = nn.Conv2d(1, 64, kernel_size=5, stride=1, padding=2)  # 修改输入通道数为1
...
# })
# result_df.to_csv('Vgg_ILM_INDEL.csv', index=False)
# print("\n结果已保存至 Vgg_ILM_INDEL.csv")
...
# 将每一行数据转换为图像（VGG16默认输入224x224，这里调整为224x224以匹配VGG的设计）
def to_image(tensor, size=(224, 224)):
    ...
    # 如果特征数量小于 size[0] * size[1]，则填充
        ...
        pad_size = size[0] * size[1] - tensor.shape[1]
        tensor = torch.cat([tensor, torch.zeros(tensor.shape[0], pad_size)], dim=1)
    return tensor.reshape(-1, 1, *size)  # 输出形状：(batch_size, 1, 224, 224)
...
# 修改输入层：将3通道卷积层替换为1通道，保持其他参数不变
# VGG16的第一个卷积层是features[0]，原参数：in_channels=3, out_channels=64, kernel_size=3, stride=1, padding=1
in_channels = 1
...
# 创建新的1通道输入卷积层，并初始化权重
new_conv1 = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, bias=bias)

# 权重初始化：将原3通道权重的平均值作为1通道的权重（保持特征提取能力）
if model.features[0].weight is not None:
    new_conv1.weight.data = model.features[0].weight.data.mean(dim=1, keepdim=True)

# 替换第一个卷积层
model.features[0] = new_conv1

# 修改全连接层的输出类别数（VGG16默认是1000类，改为2类）
num_ftrs = model.classifier[6].in_features
model.classifier[6] = nn.Linear(num_ftrs, 2)  # 替换最后一个全连接层
# =====================================================================
...
# 训练模型（VGG16参数量较大，可根据硬件情况调整epoch数）
model = train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=30)
...
result_df.to_csv('VGG16_ILM_INDEL.csv', index=False)
print("\n结果已保存至 VGG16_ILM_INDEL.csv")